...
# 15. Fenwick Tree、Segment Tree 与 Sparse Table
...
严格递增数组旋转后，任意时刻至少有一半仍然有序。比较 `values[first]`、
`values[middle]` 和目标即可判断应保留哪一半：
...
该实现要求元素互异。若允许重复，`values[first] == values[middle]` 时可能无法
判断哪边有序，只能收缩相等边界，最坏复杂度退化到 `O(n)`。
