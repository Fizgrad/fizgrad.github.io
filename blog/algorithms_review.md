...
连续区间之外，数组问题还经常需要维护前后缀状态、原地标记、投票状态或旋转后的
局部有序性，不能全部归结为同一个遍历模板。
...
- `B`：黑色，`R`：红色；
- `C`：保留操作前的颜色，可能是红色或黑色；
- `DB`：Double Black，是删除修复中的临时记号，不是第三种真实颜色；
- `N`、`F`：相对 Double Black 节点的近侄子和远侄子；
- 所有未画出的 `NIL` 叶子都视为黑色。

### 旋转的局部结构

左旋把右孩子提升为局部根；沿反方向看同一张图，就是对 `y` 做右旋：
...
`A < x < B < y < C` 的中序关系没有变化。旋转本身只改父子指针，图中的
颜色也不会自动变化；插入和删除修复会按具体情况另外重新着色。
...
先把父亲 `p` 和叔叔 `u` 染黑，再把祖父 `g` 染红。局部黑高不变，但 `g`
可能与它的父节点形成新的连续红边，因此令 `z = g` 继续向上检查：

```text
          g(B)                         g(R)  <- continue from g
         /    \                       /    \
      p(R)    u(R)   recolor        p(B)    u(B)
      /              -------->      /
    z(R)                           z(R)
```

如果 `g` 已经是根，循环结束后再把它改回黑色。

#### 情况二：叔叔为黑色，三角形先转成直线

下面是 Left-Right（LR）形。先对父节点 `p` 左旋，原来的 `z` 成为新的父节点，
结构转化成 Left-Left（LL）直线：
...
Right-Left（RL）完全镜像：先对父节点右旋，再按 Right-Right（RR）处理。

#### 情况三：叔叔为黑色，直线旋转祖父

LL 形中，把父节点染黑、祖父染红，再对祖父右旋：

```text
          g(B)                         p(B)
         /                            /    \
      p(R)      recolor + right(g)  z(R)   g(R)
      /          ---------------->
    z(R)
```

RR 形是它的镜像：父节点染黑、祖父染红，再对祖父左旋。四种方向可以归纳为：

```text
LL: right(g)             RR: left(g)
LR: left(p), right(g)    RL: right(p), left(g)
```

### 删除与 Double Black

删除仍先执行普通 BST 删除。目标节点有两个孩子时，用右子树最小节点 `y`
接替它；是否需要修复取决于真正从原位置移走的 `y` 的颜色，而不是目标节点
原来的颜色：
...
repair is determined by Cy; x may be NIL
```

删除红节点不会改变黑高。真正移走黑节点时，替代它的 `x` 所在路径会少一个
黑色，可将这种暂时的不平衡理解为 `x` 携带了一层 **Double Black**。
...
下面统一画 `x` 是左孩子的情况，因此修复主要使用左旋；`x` 在右边时，交换
所有左右方向即可。

#### 情况一：兄弟为红色

红兄弟的两个孩子必为黑色。把兄弟染黑、父亲染红，再对父亲左旋。Double
Black 尚未消失，但 `x` 得到了一个黑兄弟 `N`，从而转化为后面三种情况：

```text
          p(B)                            w(B)
         /    \                          /    \
      x(DB)   w(R)   recolor + left(p) p(R)    F(B)
              / \    ----------------> / \
           N(B) F(B)                 x(DB) N(B)
```
...
把兄弟 `w` 染红，相当于从兄弟一侧取走一层黑色，并把 `x` 的额外黑色上移
给父节点：
...
若 `p` 原来是红色，它吸收额外黑色后变成普通黑色，修复结束；若 `p` 原来是
黑色，就把 `p` 视为新的 Double Black，继续向上处理。

#### 情况三：黑兄弟的近侄子红、远侄子黑

先把近侄子 `N` 染黑、兄弟 `w` 染红，再对 `w` 右旋。新的黑兄弟拥有红色
远侄子，因此转化成情况四：

```text
          p(C)                            p(C)
         /    \                          /    \
      x(DB)   w(B)   recolor + right(w) x(DB)  N(B)
              / \    ---------------->           \
           N(R) F(B)                              w(R)
                                                    \
                                                    F(B)
...
令兄弟继承父亲原来的颜色，把父亲和远侄子染黑，再对父亲左旋。两侧黑高恢复，
Double Black 被彻底消除：

```text
          p(C)                            w(C)
         /    \                          /    \
      x(DB)   w(B)   recolor + left(p) p(B)    F(B)
              / \    ----------------> / \
           N(?) F(R)                  x(B) N(?)
```

这四步的核心不是记住颜色赋值顺序，而是持续把当前状态归约为“黑兄弟且远侄子
为红”的最终情况；若只能合并黑色，就把额外黑色向根方向传播。
...
这个过程会同时暴露算法依赖的信息、不变量和空间代价；直接套用模板容易隐藏并不成立的
输入假设。
...
维护半开区间 `[first, last)`：左边全部不满足条件，右边全部满足条件；循环每次至少
缩小一个元素，结束时 `first == last`，就是第一个满足条件的位置。
