...
同一 warp（通常 32 个线程）在一次内存访问窗口里发起请求时，硬件会按地址块（transaction）合并请求。  
所谓 coalescing，本质上是把“大量分散请求”尽量压缩成“更少、更连续的内存事务”。

特征可以归纳为：  
- 如果 warp 内线程读写的是连续、对齐的地址，调度器可在一个较窄事务序列里完成；
- 如果线程下标呈现大 stride、逆序或跨页跳跃，地址离散度上升，事务数会明显变多；
- 结果是同样吞吐下，等效每字节成本上升，延迟和带宽压力都会增大。

性能代价上体现为：  
- 全局内存本质上是按块传输的，离散访问会让一个 warp 的一轮请求占用更多事务；
- 更多事务会占用内存队列，降低 overlap 能力；
- 大量读后仍需计算，pipeline 里会出现“有用数据到达不及时”。

可行处理如下（按优先级）：

- 先改 index 映射：让同一 warp 处理连续元素（比如按行主序顺序扫描，避免每线程跳跃 `* stride` 的布局）；
- 统一对齐：基址对齐、循环起点对齐到 cache line 大小，减少首尾半事务；
- 结构化布局：连续读写时尽量 `SoA`，对按字段访问重组成 `AoS`；
- 数据重排/tiling：让同一 warp 在一个时间窗口只处理邻近地址；
- 需要时用 vector load/store（如 `float4`）与内存预取，减少指令数和事务数。

一个直觉判断法：看索引是否 `i, i+1, ...` 还是 `i, i+S, i+2S`。`S=1` 通常友好，`S` 大时需要额外验证是否有更好的映射。

### Shared Memory Bank Conflict

Shared Memory 被切成多个 bank。每个 bank 支持一个周期内的单地址访问，硬件会把一次 warp 访问映射成 `bank = (address / bank_width) mod bank_count`（不同目标可能有不同 bank_width）。  
当同一 warp 内多个线程的地址落到同一 bank 且地址不同，就会发生 bank 冲突，导致访问被分多轮完成。

它的行为特征是：  
- 与 global coalescing 一样，都是“合并资源访问”的优化问题，但发生在片上共享存储；
- 它不是错误，只是性能分摊到多个子周期；
- 同一 bank 访问同一个地址（广播）通常不一定串行，常见架构有专门的广播路径（但不应绝对假设）。

代价体现在：  
- 冲突会把原本同拍完成的共享内存读/写变成 2-way、4-way、8-way 等序列化；
- 串行化会抬升 shared memory 阶段的隐藏延迟难度，进一步放大 barrier 周期和计算停顿。

常用处理是：

- 先算 bank 映射（尤其是二维索引平铺时）：

```text
bank_id = (base_element_index + local_index) % 32  （常见 32-bank 模型）
```

- 给共享数组加 padding（例如 `TILE_W + 1`）打破步长为 bank_count 的映射；
- 采用 swizzle 或重排 index（行列换序、转置片）；
- 尽量把 `float4/half2` 这种跨 lane 的数据布局与访问模式统一，避免多线程落同 bank；
- 对不易消除的冲突，改用 register 分片或直接提升复用在 global + cache。

广播场景下，如果多个线程访问同一 shared 地址，可复用该特性；但要在目标架构验证，别把它当成通用假设。

### Warp Divergence

同一 warp 内出现 `if/else`、循环边界分化等条件差异时，不同线程会走不同路径。此时 warp 不再按一拍完成全部路径，而是按 `active mask` 分片执行。

从执行语义上看，  
- divergence 本质上是“同一个 warp 被拆成多次执行子批次”；
- 它和 branch 语义直接挂钩，通常不会改变正确性，只是把并行效率从“一拍执行”降到“多拍执行”。

这类现象带来的代价：  
- 条件路径越多，warp 的有效执行拍数越多；
- 资源利用率下降，尤其在循环内反复分支的场景；
- 与 barrier、atomic 结合时容易引发隐含的路径依赖问题。

处理思路（按代价排序）：

- 数据分区：先按条件把 warp 内线程整理成更同质的群组（例如先分流再处理）；
- 轻分支：简单条件可改用 predication 或查表/掩码运算；
- 重写流程：将分支内大块计算改为两个更“纯算子化”的 kernel 或不同 launch 版本；
- 算法层变换：例如排序/计数后再计算（减少长尾路径），或者按行长/列长分流；
- 量化收益：有些分支消除会明显增加指令数，在 compute-bound 核心上反而不划算，需 profile 验证。

一句实操原则：  
**优先降低分支带来的 warp 拆分次数，再考虑减少指令数**。如果分支几乎不重叠且热点路径已稳定，优先用 `predication`；如果每次迭代都反复分支，通常更适合拆 kernel 或预处理分流。
