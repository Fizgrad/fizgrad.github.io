...
程序计数器用于记录**当前线程正在执行的 JVM 字节码指令地址**。
...
HotSpot 会从年龄 1 开始累计各年龄对象的大小。当累计值超过目标 Survivor 大小时，当前年龄会成为新的晋升阈值；年龄大于等于该阈值的对象可以晋升老年代。该阈值还会受到 `MaxTenuringThreshold` 限制。
