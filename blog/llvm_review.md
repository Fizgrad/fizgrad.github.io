...
### 6.8.1 Hoist、Sink 与 Speculation

| 变换 | 含义 | 主要约束 |
| --- | --- | --- |
| Hoist | 把指令移动到更早的位置，例如循环 preheader | 新位置必须支配原有使用，且不能改变内存和异常语义 |
| Sink | 把指令移动到更晚的位置 | 新位置仍要覆盖所有必要使用，并保持副作用顺序 |
| Speculation | 让指令在原本可能不执行的路径上也执行 | 指令必须可安全提前执行，不能新增 trap、UB 或不可接受的 poison 观察 |
