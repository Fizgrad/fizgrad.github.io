...
目标：从 fd 就绪一直跟到 gRPC 任务和响应写回。

### 第三遍：看消息事务

```text
message_service/src/MessageServiceImpl.cpp
common/dao/MessageDao.cpp
common/conversation/ConversationDao.cpp
common/outbox/
deploy/mysql/migration/
```
...
```text
push_service/src/PushWorker.cpp
common/push/PushDispatcher.cpp
common/push/PushRetryManager.*
common/kafka/
```

目标：理解 offset、retry、DLQ、offline 和在线投递结果。
...
最终应能从“客户端发一个二进制包”一路讲到“接收方 ACK 后数据库状态如何变化”，并能准确指出每一步的失败处理和一致性语义。
