...
但“单体内核”不等于“所有代码都写在一个文件中”。Linux 在代码组织和运行机制上具有明显的模块化设计：
...
| 配置  | 含义                    | 运行时特点                |
| --- | --------------------- | -------------------- |
| `y` | Built-in，内建           | 编译进内核镜像，启动时即可使用      |
| `m` | Loadable Module，可加载模块 | 编译为 `.ko` 文件，可在运行时加载 |
| `n` | Disabled，禁用           | 内核中不包含该功能            |
...
| 对比点    | 用户态动态库 `.so`     | 内核模块 `.ko`          |
| ------ | ---------------- | ------------------- |
| 运行位置   | 用户空间             | 内核空间                |
| 装载对象   | 某个进程             | 整个运行中的内核            |
| 常见加载方式 | 动态链接器、`dlopen()` | `modprobe`、`insmod` |
| 可调用接口  | libc 和用户态库       | 内核导出的接口             |
| 故障影响   | 通常影响当前进程         | 可能影响整个系统            |
| 权限     | 普通进程权限           | 内核最高权限              |
| 标准 C 库 | 可以使用             | 不能直接使用用户态 libc      |

模块代码不能调用用户态的 `printf()`、`malloc()` 或 `pthread_mutex_lock()`。它需要使用对应的内核接口，例如：

```text
printf       → pr_info / printk
malloc       → kmalloc / vmalloc
free         → kfree / vfree
pthread锁    → 内核 mutex / spinlock
```
...
如果代码直接编译进内核，`module_init()` 对应的初始化函数会在内核初始化阶段执行；如果代码编译为模块，则在插入模块时执行。`module_exit()` 主要用于可卸载模块，内建代码不会在系统运行期间被作为独立模块卸载。
...
`lsmod` 格式化显示 `/proc/modules` 中记录的当前已加载模块。
...
`modinfo` 会从当前内核版本对应的模块目录中读取模块元数据。
...
`modprobe` 会处理模块依赖和 `/etc/modprobe.d/` 中的配置。
...
`insmod` 只负责插入给定模块文件，不会像 `modprobe` 一样完整处理依赖，因此一般优先使用 `modprobe`。加载失败时，具体原因经常需要结合 `dmesg` 查看。
...
`rmmod` 是较直接的移除工具；`modprobe -r` 还可以处理已经不再需要的依赖模块。模块必须支持卸载，并且不能仍被其他代码或设备使用。
...
内核使用导出符号机制控制模块可以引用的函数和变量。常见宏包括：
...
只有被导出的内核符号才能供其他模块进行正常符号解析。内核还支持符号命名空间，用于进一步组织导出接口。
...
如果模块依赖的符号不存在，或者相关依赖模块未加载，模块加载通常会失败，并在内核日志中出现类似：
...
MODULE_PARM_DESC(debug_level, "Debug output level");
```

加载时可以传入：

```bash
sudo modprobe demo debug_level=2
```

也可以使用：

```bash
sudo insmod demo.ko debug_level=2
```
...
Linux 支持模块签名：模块可以在安装阶段进行数字签名，内核在加载时验证签名；系统也可以配置为拒绝无效签名或未签名模块。

加载专有模块、树外模块或发生某些异常后，内核可能被标记为 **tainted**。该状态主要用于故障分析，提醒开发者当前内核运行环境包含可能影响问题判断的因素。
...
Linux 核心子系统和已加载模块仍在同一内核地址空间运行。模块化解决的是代码组织、按需加载和功能扩展，不自动提供微内核式的进程隔离。

#### “模块被卸载后，内核会自动清理所有资源”

错误。

模块必须在退出函数中正确注销接口并释放资源。内核不能自动理解模块申请的所有业务资源。

#### “`.ko` 可以加载到任意 Linux 内核”

错误。

模块通常需要和目标内核的架构、配置、构建信息以及可用符号匹配。外部模块应使用目标内核对应的构建环境编译。

#### “`insmod` 和 `modprobe` 完全相同”

错误。

`insmod` 直接插入指定文件；`modprobe` 会按模块名查找模块、读取配置并处理依赖。
...
* Linux 是单体内核，但内部按子系统和接口模块化组织；
* 功能可以内建、编译成模块或完全禁用；
* `.ko` 运行在内核空间，不是普通用户态动态库；
* `modprobe` 负责查找和处理依赖，`insmod` 直接加载文件；
* `module_init()` 和 `module_exit()` 定义模块生命周期；
* 模块通过导出符号和注册接口接入内核子系统；
* 模块参数可通过加载命令或 sysfs 配置；
* 正在使用的模块通常不能正常卸载；
* 模块错误可能影响整个系统；
